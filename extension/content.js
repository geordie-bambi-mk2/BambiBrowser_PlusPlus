...
if (window.__bambiLoaded) {
  ...
  function checkDomainMatch(domains) {
    return domains.some(d => hostname.includes(d.toLowerCase()));
  }
  ...
  function isBambiActivated() {
    return bambiActivated && isMatchedDomain && !isBlacklisted();
  }
  ...
  function findMainVideo() {
    ...
    // Generic fallback: pick the largest video by area
    candidates.sort((a, b) => {
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.type === "BAMBI_ACTIVATE" && isMatchedDomain && !isBlacklisted()) {
      console.log("[Bambi] received BAMBI_ACTIVATE → triggering hijack");
